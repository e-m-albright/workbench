...
async function runCli(
  ...
  if (result.code !== 0) throw new Error(result.stderr.trim() || result.stdout.trim() || "Apple Notes failed.");
...
export default function appleNotesExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    description: "List readable, unshared Apple Notes folders. Password-protected and shared content is excluded.",
  ...
  pi.registerTool({
    ...
    label: "Apple Note",
    description: "Read one unshared, unlocked Apple Note by stable note id. Output is truncated to 20,000 characters.",
