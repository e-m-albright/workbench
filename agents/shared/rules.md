...
- **Deliver paste-ready copy through the clipboard.** When the user asks for an email, message, or replacement wording intended for pasting, copy the finished plain text to the local clipboard by default when available, then explicitly say what was copied. Preserve paragraph breaks without terminal padding or manual line wrapping. On macOS, use `pbcopy` from a clean text file and verify with `pbpaste` against that file before claiming success. Copy only the requested deliverable, not surrounding analysis; for hand edits, preserve the user's draft and copy only the requested replacement passages. Do not automatically copy secrets, provisional alternatives, or ordinary analytical replies. If clipboard access fails, say so and offer the clean file or the harness's raw-message copy command (Pi: `/copy`).
...
- **Hand off blocked deletions explicitly.** When a required deletion is unavailable through agent tools or blocked by policy, give the user the exact safest command to run instead of merely reporting that something remains. Prefer `rmdir <path>` for an expected empty directory so the command fails safely if contents remain; explain any broader or destructive command before presenting it.
