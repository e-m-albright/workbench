---
name: prototype
description: Build a throwaway prototype to answer a design question before committing. Use for interactive state-model experiments, rapid UI variants, mockups, or "let me try a few approaches".
metadata:
...
2. **One command to run.** Whatever the project's existing task runner supports — `deno task <name>`, `python <path>`, `cargo run`, etc. The user must be able to start it without thinking.
