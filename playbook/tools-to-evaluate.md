...
- **[Namespace](https://namespace.so)** -- **WATCH (reviewed 2026-09-02).** Developer compute spanning drop-in GitHub Actions runners on Linux, Windows, and macOS; ephemeral Docker-capable Devboxes for coding agents; and remote caching for Docker, Bazel, Turbo, and Nix. Its interesting distinction from runner-only services is one infrastructure layer for CI and isolated agent workspaces. Trial only when CI queue/build time or safe remote agent execution becomes a recurring bottleneck; benchmark one representative workflow against GitHub-hosted runners and Depot, including cache behavior, debugging, source/secret isolation, egress controls, and total cost. Sources: [overview](https://namespace.so/), [GitHub Actions setup](https://namespace.so/docs/solutions/github-actions), [agent Devboxes](https://namespace.so/docs/devbox/agents), and [pricing](https://namespace.so/pricing).
...
- **[GitHub pull-request dashboard](https://github.blog/changelog/2026-07-09-new-pull-requests-dashboard-is-now-generally-available/)** — **USE THE EXISTING NATIVE INBOX before adding review-queue software.** The generally available dashboard centralizes requested reviews, failing or newly commented pull requests, ready-to-merge work, merge-queue state, and saved cross-repository views. It does not assess risk or review code, but it addresses the attention-allocation problem without another vendor. Create saved views for stale requests, high-risk labels, and ready-to-merge work when review volume becomes difficult to scan.
...
- **[Greptile](https://www.greptile.com)** -- AI code-review agent that builds a graph index of the codebase and runs parallel agents to review pull requests with repository context. Evaluate its findings against the existing review workflow before adoption; vendor benchmark claims do not establish local value.
...
### Design-system and UI verification (reviewed 2026-09-24)

- **[shadcn/lint](https://github.com/shadcn-ui/lint)** — **EVALUATE for a Tailwind v4 project with recurring agent-caused design-system drift; do not add globally.** Version 0.2.0 is an MIT ESLint/Oxlint plugin with six deterministic rules for component restyling, raw colors, arbitrary values, inline styles, unknown classes, and dynamic classes. Its useful distinction is actionable diagnostics built from the project's components, variants, and theme. The [primary documentation](https://github.com/shadcn-ui/lint#readme) now documents React, Vue, and Svelte support, correcting the earlier secondary review: ESLint parses Vue and Svelte markup, while Oxlint sees only their script blocks. Upstream reports more than 150 agent task runs and near-universal correction within one round, but that is a first-party benchmark. Trigger: a Tailwind project has a named token or component-contract violation that Biome and framework checks miss; trial one violating and one valid fixture per enabled rule before adding the plugin. No Biome integration is documented.
- **[eslint-plugin-better-tailwindcss](https://github.com/schoero/eslint-plugin-better-tailwindcss)** — **EVALUATE CONDITIONALLY.** Upstream [Svelte parser documentation](https://github.com/schoero/eslint-plugin-better-tailwindcss/blob/main/docs/parsers/svelte.md) explicitly describes Svelte integration. Candidate for Tailwind class correctness; not evidence of component-contract enforcement equivalent to shadcn/lint. Requires ESLint plus the Svelte parser, which conflicts with the default desire for one broad Biome toolchain. Trial only when concrete missing rules justify that exception; verify Tailwind v4 behavior, dynamic classes, maintenance, license, and false positives before adoption.
- **[Stylelint](https://stylelint.io/)** — **WATCH for token/CSS policy gaps.** Candidate for restrictions on raw CSS values, properties, and custom conventions when Biome cannot enforce a named design-system rule. Check actual Svelte style-block parser support and rule packages before adding it. Avoid duplicating generic CSS lint already covered by Biome. No package installation or local trial performed.
- **[axe-core with Playwright](https://playwright.dev/docs/accessibility-testing)** — **USE THE EXISTING STACK PICK**, not a new watchlist dependency. Runtime accessibility checks complement Svelte compiler checks and token lint; they do not prove all accessibility or visual quality. Add checks to representative rendered pages plus explicit keyboard/focus tests. Keep visual snapshots for layout regressions separate from semantic accessibility assertions.

Evaluation protocol: seed one real violation and one valid example per promised rule; verify the selected linter parses the relevant framework markup rather than only script blocks; inspect actionable diagnostics and safe-fix behavior; record overlap and runtime cost. Do not trade framework semantic checking for a narrower design-system linter, or treat generated-design taste as objectively proved by static analysis.
...
- **[Tauri](https://v2.tauri.app/start/)** remains the default when a web frontend needs native desktop distribution. It reuses the operating-system webview, has the strongest fit with the existing Svelte and Rust guidance, and already has an owned stack page. Better Stack's [Tauri, Electrobun, and Deno Desktop comparison](https://www.youtube.com/watch?v=ktkDGQyRx3Y) was a useful survey, but did not expose a current desktop-app need or displace Tauri.
- **[Wails](https://wails.io/docs/introduction/)** is the equivalent Go-shaped choice: system webviews, Go methods exposed to JavaScript, generated TypeScript models, and Svelte templates. Better Stack's [screen-recorder comparison](https://www.youtube.com/watch?v=Q1TL2AKoy00) found it credible but showed the practical ecosystem boundary: native platform APIs may require custom Cgo and Objective-C when the Go package ecosystem has no wrapper. Use Wails when Go is already the application's center of gravity; do not introduce Go merely to avoid Rust in Tauri.
- **Deno Desktop and Electrobun** are watch-only. Their cohesive web-toolchain story is attractive, but a framework choice starts from a desktop product and its native integration requirements, not from wanting to try another runtime.
...
- **[Nub](https://nubjs.com/)** -- Rust-built toolkit layered over stock Node.js: TypeScript file and script running, Node version management, package installation, `nubx`, watch mode, and package-manager shims without a new runtime or lockfile format. Clever consolidation, but it currently duplicates owners that are working: exact Node versions, Corepack-pinned pnpm, project scripts, committed lockfiles, and Just gates. The recent Notes revamp made those boundaries more explicit and reduced the web gate's cost; replacing them for claimed startup or install speed would widen the trust surface without removing a measured bottleneck. Watch for maturation, but trial only in a new Node-heavy repository where several of those tools cause recurring friction. Source overview: [Better Stack video](https://www.youtube.com/watch?v=6YRpXxbtc2c).
...
- **Multi-tenant platform stack** -- Track k3s + Cilium + Kamaji + Capsule only for a product or internal platform that must provide many isolated tenant control planes on shared infrastructure. The mechanism and adoption trigger live in [the infrastructure landscape](knowledge/infrastructure-as-code-landscape.md#multi-tenant-platform-stack-to-recognize).
...
- **[Jev and System One Models](https://typesafe.ai/blog/introducing-system-one-models-and-jev)** -- **WATCH (reviewed 2026-09-19).** TypeSafe AI positions Jev as a hosted decision model rather than a chat or generation model: unstructured state goes in and a predefined typed value, probabilities, and confidence come out in one parallel sample. The first-party announcement reports early access, input pricing of $0.042 per million tokens with unmetered output, 70–500 millisecond latency, and choice cardinality up to 255. This is not a replacement for Pi, Claude Code, or Codex. Its plausible utility is as a low-latency classifier, router, scorer, extractor, guardrail, or verifier inside a high-volume application workflow. “Cannot hallucinate” is too broad: schema-constrained output can eliminate malformed or invented fields while remaining semantically wrong, poorly calibrated, or wrong under distribution shift. The published workflow evaluation uses the average predictions of larger proprietary models as its reference rather than ground truth, was produced internally, and has no independent replication; pricing sustainability, privacy, licensing, and training-data details also remain unresolved. Do not adopt for the current interactive coding workflow. Revisit when an owned production system needs many typed probabilistic decisions under a strict latency or cost budget, then measure Brier score or calibration error, task accuracy, abstention behavior, out-of-distribution degradation, tail latency, and total cost against ordinary structured-output models and deterministic rules.
...
- **[Recall](https://github.com/raiyanyahya/recall)** -- Local cross-session memory that stores complete session transcripts and deterministic extractive summaries in repository Markdown. **Reference only; do not adopt.** The inspectable local files are better than opaque hosted memory, but append-only transcripts can retain secrets, stale assumptions, and large amounts of private context. Workbench's intentional handoff artifacts provide a narrower lifecycle and clearer authority boundary. Revisit only if repeated continuity failures survive better handoffs and the candidate adds retention, redaction, supersession, and repository-specific scope.
- **[Vercel AI SDK](https://ai-sdk.dev)** -- **Graduated:** the TypeScript stack owns the default in [`stacks/typescript/README.md`](stacks/typescript/README.md), while features, competitors, Vercel ownership, and gateway relationships live in [`stacks/vercel-cloudflare.md`](stacks/vercel-cloudflare.md#ai-sdk-features-and-competitors). Do not maintain a second volatile feature inventory here.
- **[LangExtract](https://github.com/google/langextract)** -- Google-authored Apache-2.0 Python library for prompt-defined structured extraction from long documents. It chunks and processes documents in parallel, maps extracted values back to exact source spans, flags values it cannot align, and emits an interactive review artifact; model support includes Gemini, OpenAI, and local Ollama providers. The repository explicitly says it is not an officially supported Google product, and source alignment does not prove that the extracted interpretation is correct. **Verdict: watch for evidence-heavy document ingestion**, especially clinical or regulatory text; benchmark recall, span accuracy, schema consistency, and human-review burden against deterministic parsing plus the current model stack before adoption.
...
- **[Mindwalk](https://github.com/cosmtrek/mindwalk)** -- Local Go tool that maps a repository into a 3D city and replays Claude Code, Codex, and Pi session logs as searched, read, and edited paths. The visual metaphor is partly novelty, but the underlying question is legitimate: did an agent inspect the right neighborhood before changing it? Keep it as a learning and session-review experiment for an unfamiliar, sprawling repository, not as a standing gate. Git diff, tests, and deterministic scope checks remain faster for routine changes; optional model evaluation sends a session summary through the configured Claude or Codex CLI. Source overview: [Better Stack video](https://www.youtube.com/watch?v=Y_7_rbtQXcs).
...
- **[Heretic](https://github.com/p-e-w/heretic)** (AGPL-3.0) -- Automates "abliteration," a research technique that identifies and removes refusal-related directions from open-weight transformer models. Interesting as a model-behavior and safety experiment, not as a normal assistant configuration: weakening refusals expands misuse risk and may damage unrelated capability. If explored, use an isolated local model with no private data or external tools, compare capability and safety behavior before and after, and treat the output as an untrusted research artifact. Do not deploy or route ordinary work through it.
...
Personal vibes-check on the current player set (2026-05). Opinions, not benchmarks — refine as we get hands-on. The maintained product-by-product comparison now lives in [`stacks/vercel-cloudflare.md`](stacks/vercel-cloudflare.md); keep these bullets as the broader provider shortlist rather than duplicating that inventory.
