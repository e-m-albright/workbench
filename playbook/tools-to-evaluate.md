...
- **[Handy](https://github.com/cjpais/Handy)** (MIT; macOS/Windows/Linux; 27.5k stars; active Jul 2026) -- The strongest FOSS general-purpose alternative: offline Whisper.cpp/Parakeet dictation, shortcut-driven, cross-platform, and by far the largest community here. Its core value is reliable raw transcription rather than an AI rewrite layer. Evaluate only if TypeWhisper becomes limiting or cross-platform parity matters.
...
- **[Hugging Face speech-to-speech](https://github.com/huggingface/speech-to-speech)** (Apache-2.0; 6.4k stars; active Jul 2026) -- Best architecture bet and the only candidate worth a deliberate prototype: modular VAD → STT → text LLM → TTS, OpenAI Realtime-compatible WebSocket/WebRTC, interruption and tool-call events, Apple-Silicon MLX paths, and local/hosted OpenAI-compatible LLM backends including llama.cpp. It is production-backed by Reachy Mini, but remains infrastructure rather than a coding-agent UI. Use it later as the speech transport behind a thin Pi SDK/RPC client, not as another agent stack.
...
**Recommended experiment when voice becomes more than curiosity:** build a disposable, local Pi extension around the tools already present: record from a Pi shortcut, transcribe through TypeWhisper's bundled CLI/localhost API, inject with `pi.sendUserMessage(..., { deliverAs: "steer" })`, and speak finalized assistant text with macOS `say`. Add shortcut-driven cancellation of `say`/the active turn for barge-in. This is smaller and more native than MCP; if that UX proves valuable, replace only the speech transport with HF speech-to-speech. Define the trial in `docs/experiments.md` before implementation.
