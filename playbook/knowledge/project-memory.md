...
`CONTEXT.md` is a project convention (DDD-flavored), not a tool-supported standard the way `AGENTS.md` is — only workflows such as planning's documentation grill and `converge` read it by name. The pointer in AGENTS.md is what makes it discoverable.
